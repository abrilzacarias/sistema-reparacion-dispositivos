...
router = APIRouter(prefix="/puestosLaborales", tags=["Puestos Laborales"])

@router.get("/", response_model=List[schemas.PuestoLaboralOut])
def get_all_puestos(db: Session = Depends(get_db)):
    return services.get_all_puestos(db)
